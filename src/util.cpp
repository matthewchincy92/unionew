...
const char * const BITCOIN_CONF_FILENAME = "unio.conf";
const char * const BITCOIN_PID_FILENAME = "uniod.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "unio" is a composite category enabling all Unio-related debug output
            if(ptrCategory->count(string("unio"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\UnioCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\UnioCore
    // Mac: ~/Library/Application Support/UnioCore
    // Unix: ~/.uniocore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "UnioCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/UnioCore";
#else
    // Unix
    return pathRet / ".uniocore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty unio.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override unio.conf

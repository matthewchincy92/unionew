...
*Setup instructions for a Gitian build of Unio Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Unio
...
- [Building Unio Core](#building-unio-core)
...
Clone the git repositories for Unio Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/uniocoin/unio
```
...
in the Unio Core repository under 'Fetch and build inputs' to install sources which require
...
To build Unio Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Unio Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/unio/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/uniocoin/unio
...
For example:
```bash
URL=https://github.com/crowning-/unio.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit unio=${COMMIT} --url unio=${URL} ../unio/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit unio=${COMMIT} --url unio=${URL} ../unio/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit unio=${COMMIT} --url unio=${URL} ../unio/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the unio git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../unio/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/unio.git
SIGPATH=/some/root/path/unio-detached-sigs.git

./bin/gbuild --url unio=${BTCPATH},signature=${SIGPATH} ../unio/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/unio-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/unio-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/unio-osx-build.assert
```
...
[unio/gitian.sigs](https://github.com/uniocoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
